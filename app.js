...
function autosaveItem() {
    ...
    if (currentEditingType === 'brief') {
        ...
        const newProposition = document.getElementById('editorProposition').value.trim();
        const newClientBrief = document.getElementById('editorClientBrief').value.trim();
...
function setupAutosaveListeners() {
    ...
    // Brief fields
    const propositionField = document.getElementById('editorProposition');
    const clientBriefField = document.getElementById('editorClientBrief');
...
window.openTaskSourceFromSummary = function(taskId, projectName) {
    console.log('Opening task source from summary:', taskId, projectName);
    // Find the project
    const project = projects.find(p => p.name === projectName);
    if (!project) {
    ...
    // Switch to the project if it's not current
    if (!currentProject || currentProject.id !== project.id) {
        currentProject = project;
        document.getElementById('projectSelect').value = project.id;
        document.getElementById('dashboard').style.display = 'grid';
    ...
    // If task has no source, just open the task editor
    if (!task.sourceItemId || !task.sourceItemType) {
        console.log('Task has no source, opening task editor');
        setTimeout(() => {
            openItemEditor(task, 'task');
        }, 200);
        return;
    ...
    if (sourceItem) {
        console.log('Found source item in project:', sourceItem);
        setTimeout(() => {
            openItemEditor(sourceItem, task.sourceItemType);
        }, 200);
    } else {
        console.log('Source item not found in project, opening task editor instead');
        setTimeout(() => {
            openItemEditor(task, 'task');
        }, 200);
    }
...
window.diveInToSource = function(taskId, projectName) {
    console.log('Diving into source from summary:', taskId, projectName);
    // Find the project
    const project = projects.find(p => p.name === projectName);
    if (!project) {
        console.log('Project not found:', projectName);
...
function startPomodoro() {
    ...
    // Enter fullscreen mode during focus sessions (not breaks)
    if (!pomodoroIsBreak) {
        enterFocusMode();
    }
...
function resetPomodoro() {
    pausePomodoro();
    pomodoroIsBreak = false; // Always reset to work session
    pomodoroTimeLeft = 25 * 60; // Always reset to 25 minutes
...
function completePomodoro() {
    ...
    if (pomodoroIsBreak) {
        ...
        // Offer to resume work if there was context saved
        if (workContext.currentContext) {
            setTimeout(() => {
                const shouldResume = confirm('Break complete! Resume your previous work?');
                if (shouldResume && workContext.currentContext) {
...
function initializePomodoro() {
    ...
    if (savedDaily) {
        ...
        if (dailyData.date === today) {
            pomodoroDailyCount = dailyData.count;
        } else {
    ...
    if (projectId) {
        ...
        if (projectContext && projectContext.editorState) {
            ...
            setTimeout(() => {
                ...
                // Only offer restoration if context is recent (within 4 hours)
                if (timeDiff < 4 * 60 * 60 * 1000) {
                    const shouldRestore = confirm(`Resume your previous work on "${projectContext.title}"?`);
                    if (shouldRestore) {
...
function openItemEditor(item, itemType) {
    ...
    // Populate editor
    document.getElementById('editorTitle').textContent = `Edit ${itemType.charAt(0).toUpperCase() + itemType.slice(1)}`;
    ...
    const insertHeadingsBtn = document.getElementById('insertHeadingsBtn');
    ...
    if (itemType === 'brief') {
        ...
        insertHeadingsBtn.style.display = itemType === 'note' ? 'inline-flex' : 'none';
        
        // Show copy to clipboard button for notes and copy
        copyToClipboardBtn.style.display = (itemType === 'note' || itemType === 'copy') ? 'inline-flex' : 'none';
    }
    ...
    const setupPomodoroTimer = () => {
        ...
        if (itemType === 'note' || itemType === 'copy') {
            if (pomodoroTimer) {
                ...
                setTimeout(() => {
                    const retryTimer = document.getElementById('pomodoroTimer');
                    if (retryTimer) {
    ...
    setTimeout(() => {
        ...
        if (existingContext && 
            ...
            // Ask if user wants to restore previous state
            const shouldRestore = confirm(`Restore your previous work on "${item.title}"? This will restore your cursor position and unsaved changes.`);
            if (shouldRestore) {
                setTimeout(() => {
    ...
    list.innerHTML = sortedCopy.map(copy => {
        ...
        return `
            <div class="item copy-item sortable-item ${isLinked ? 'linked-item' : ''}" 
                ...
                <div class="item-meta">
                    Created: ${new Date(copy.createdAt).toLocaleDateString()}
                    ${isLinked && linkedBrief ? ` • Linked to "${linkedBrief.title}"` : ''}
                </div>
                ${copy.content ? `
                    <div style="margin: 8px 0; color: #525252; line-height: 1.4;">
                        ${copy.content.substring(0, 100)}${copy.content.length > 100 ? '...' : ''}
                    </div>
                ` : ''}
                <div class="item-actions">
                    <div style="font-size: 11px; color: #a3a3a3; font-style: italic; flex: 1;">
...
function toggleProjectTask(taskId) {
    ...
    if (task) {
        ...
        if (task.completed) {
            ...
            // Remove completed tasks from priority lists immediately
            const uniqueId = `${currentProject.id}-${taskId}`;
            const wasInTopThree = globalTaskOrder.topThree.includes(uniqueId);
            const wasInOther = globalTaskOrder.other.includes(uniqueId);
...
window.diveInToProjectSource = function(taskId) {
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'R' && e.ctrlKey && e.shiftKey) {
        e.preventDefault();
        if (workContext.currentContext) {
            const shouldRestore = confirm(`Resume work on "${workContext.currentContext.title}"?`);
            if (shouldRestore) {
